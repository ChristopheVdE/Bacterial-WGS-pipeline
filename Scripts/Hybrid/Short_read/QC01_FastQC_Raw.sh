...
#CREATE OUTPUTFOLDER IF NOT EXISTS--------------------------------------------------------------------------
     mkdir -p /home/Pipeline/Hybrid/${Run}/Short_reads/${id}/01_QC-Rawdata/QC_FastQC
#RUN FASTQC-------------------------------------------------------------------------------------------------
     for i in $(ls /home/Pipeline/Hybrid/${Run}/Short_reads/${id}/00_Rawdata | grep fastq.gz); do
          echo -e "STARTING ${i} \n";
          fastqc --extract \
          -t ${Threads} \
          -o /home/Pipeline/Hybrid/${Run}/Short_reads/${id}/01_QC-Rawdata/QC_FastQC \
          /home/Pipeline/Hybrid/${Run}/Short_reads/${id}/00_Rawdata/${i} \
          2>&1 | tee -a /home/Pipeline/Hybrid/${Run}/Short_reads/${id}/01_QC-Rawdata/QC_FastQC/stdout_err.txt ;
          echo -e "\n ${i} FINISHED \n";
     done
done
#==============================================================================================================
